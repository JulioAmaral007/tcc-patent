...
export function DetailItem({ label, value, isHighValue, isLast }: DetailItemProps) {
  const [isExpanded, setIsExpanded] = useState(false)
  const cleanLabel = label.replace(/[├└]──\s*/, '').trim()
  
  // Detecta se é um resumo ou abstract
  const isSummary = cleanLabel.toLowerCase().includes('resumo') || 
  ...
  // Se for resumo e tiver um tamanho considerável, permite expandir
  // Também remove o '...' que pode vir do backend se estivermos expandindo
  const displayValue = isSummary && isExpanded ? value.replace(/\.\.\.$/, '') : value
    ...
    <div className="flex gap-3 group/line hover:bg-white/5 rounded px-2 -mx-2 transition-colors">
      ...
      <div className="flex flex-col gap-1 flex-1 min-w-0">
        <div className="flex flex-wrap items-baseline gap-1.5 overflow-hidden">
          <span className="whitespace-nowrap text-muted-foreground/80">{cleanLabel}:</span>
          <span className={cn(
            "font-semibold transition-all duration-300",
            isHighValue ? "text-primary" : "text-foreground text-sm",
            isSummary && !isExpanded ? "line-clamp-2" : (!isSummary ? "truncate" : "whitespace-pre-wrap")
          )}>
            {displayValue}
          </span>
        </div>
